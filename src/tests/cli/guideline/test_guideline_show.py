...
def test_guideline_show_success(runner):
    ...
    with runner.isolated_filesystem():
        ...
        assert "Displaying Built-in Guideline: pm" in result.stdout
        assert "Welcome to the PM Tool!" in result.stdout
...
def test_guideline_show_not_found(runner):
    ...
    with runner.isolated_filesystem():
        ...
        assert "Error: Guideline 'nonexistent' not found." in result.stderr
...
def test_guideline_show_custom(runner):
    ...
    with runner.isolated_filesystem() as fs:
        ...
        assert result.exit_code == 0
        assert "Displaying Custom Guideline: show-custom" in result.stdout
        # Check for raw content (rich rendering removed)
        assert "Custom **Show** Content" in result.stdout
        # Ensure frontmatter isn't shown in content
        assert "description:" not in result.stdout
        assert "Show Desc" not in result.stdout
...
def test_guideline_show_prefers_custom_over_builtin(runner):
    ...
    with runner.isolated_filesystem() as fs:
        ...
        _create_guideline_file(fs_path, "default", "My **Local** Default Content")
        result = runner.invoke(cli, ["guideline", "show", "default"])
        assert result.exit_code == 0
        assert "Displaying Custom Guideline: default" in result.stdout
        assert "My **Local** Default Content" in result.stdout
        # Ensure built-in content isn't shown
        assert "Welcome to the PM Tool!" not in result.stdout
...
def test_guideline_show_builtin_when_no_custom(runner):
    ...
    with runner.isolated_filesystem():  # No custom files created
        ...
        assert result.exit_code == 0
        assert "Displaying Built-in Guideline: coding" in result.stdout
        # Check for content from actual built-in coding.md
        assert "Coding Practices" in result.stdout
        assert "Follow the project's coding standards." in result.stdout
...
def test_guideline_show_error_not_found_anywhere(runner):
    ...
    with runner.isolated_filesystem():
        result = runner.invoke(cli, ["guideline", "show", "completely-nonexistent"])
        assert result.exit_code != 0
        assert "Error: Guideline 'completely-nonexistent' not found." in result.stderr
