...
def test_guideline_list_success(runner):
    ...
    with runner.isolated_filesystem():
        ...
        assert "Scanning for guidelines..." in result.stdout
        assert "Available Guidelines:" in result.stdout
        # Check against actual descriptions from files (adjust if they change)
        assert (
            "- coding [Built-in]: Standards and conventions for writing code within this project."
            in result.stdout
        )
        # Changed 'default' to 'pm'
        assert (
            "- pm [Built-in]: General usage guidelines, core commands, and session workflow for the PM tool."
            in result.stdout
        )
        assert (
            "- testing [Built-in]: Best practices for writing and maintaining tests for the project."
            in result.stdout
        )
        assert (
            "- vcs [Built-in]: Guidelines for using version control (Git), including branching and commit strategies."
            in result.stdout
        )
...
def test_guideline_list_no_guidelines(mock_find_root, mock_resources_dir, runner):
    ...
    assert "No guidelines found." in result.stdout
    # Ensure the header isn't printed
    assert "Available Guidelines:" not in result.stdout
...
def test_guideline_list_no_description(mock_find_root, mock_resources_dir, runner):
    ...
    with patch("pm.core.guideline.frontmatter.load", side_effect=mock_load_side_effect):
        ...
        assert "- nodesc [Built-in]: No description available." in result.stdout
...
def test_guideline_list_parsing_error(mock_find_root, mock_resources_dir, runner):
    ...
    with patch("pm.core.guideline.frontmatter.load", side_effect=mock_exception):
        ...
        assert (
            "Could not parse metadata from built-in welcome_guidelines_invalid.md"
            in result.stdout
        )
        assert "Mock parsing error" in result.stdout
        # Check that no guidelines list is printed if only errors occurred
        assert "Available Guidelines:" not in result.stdout
        assert "No guidelines found." in result.stdout  # Message changed
...
# Patch find_project_root used within the core guideline module
@patch("pm.core.guideline.find_project_root")
def test_guideline_list_mixed_success_and_error(
    mock_find_root, mock_resources_dir, runner
):
    ...
    mock_resources_dir.glob.return_value = [mock_default_path, mock_invalid_path]
    # Mock find_project_root to return None (no project found)
    mock_find_root.return_value = None

    # Mock frontmatter.load: succeed for default, fail for invalid
    mock_exception = Exception("Mock YAML parsing error")  # Use generic Exception
    ...
    with patch("pm.core.guideline.frontmatter.load", side_effect=mock_load_side_effect):
        ...
        assert "Mock YAML parsing error" in result.stdout
        ...
        assert "- default [Built-in]: Mock Default Description" in result.stdout
...
def test_guideline_list_shows_custom(runner):
    ...
    with runner.isolated_filesystem() as fs:
        ...
        assert "- my-list-test [Custom]: Custom Desc" in result.stdout
        # Also check a built-in one is still listed (use partial match for flexibility)
        # Changed 'default' to 'pm'
        assert "- pm [Built-in]: General usage guidelines" in result.stdout
...
def test_guideline_list_custom_overrides_builtin_name(runner):
    ...
    with runner.isolated_filesystem() as fs:
        ...
        assert "- coding [Custom]: Local Coding Rules" in result.stdout
        # Make sure built-in isn't also listed by checking absence of its type marker
        assert "- coding [Built-in]:" not in result.stdout
...
def test_guideline_list_multiple_custom_and_builtin(runner):
    ...
    with runner.isolated_filesystem() as fs:
        ...
        _create_guideline_file(fs_path, "alpha-custom", "A", {"description": "Alpha"})
        _create_guideline_file(fs_path, "zeta-custom", "Z", {"description": "Zeta"})
        _create_guideline_file(
            fs_path, "testing", "Local Tests", {"description": "Local Testing Rules"}
        )
        ...
        assert output.find("coding") < output.find("pm")  # Changed 'default' to 'pm'
...
def test_guideline_list_only_custom(runner):
    ...
    with patch("pm.core.guideline.RESOURCES_DIR", mock_resources_dir):
        with runner.isolated_filesystem() as fs:
            ...
            # Use corrected helper
            _create_guideline_file(
                fs_path, "only-custom", "Content", {"description": "Only"}
            )
            result = runner.invoke(cli, ["guideline", "list"])

            assert result.exit_code == 0
            # Check custom guideline has correct description
            assert "- only-custom [Custom]: Only" in result.stdout
            assert "[Built-in]" not in result.stdout
            # Verify the mocked glob was called as expected on the mock object
            mock_resources_dir.glob.assert_called_once_with("welcome_guidelines_*.md")
...
def test_guideline_list_no_custom(runner):
    ...
    with runner.isolated_filesystem():  # No custom files created
        ...
        # Changed 'default' to 'pm'
        assert "- pm [Built-in]: General usage guidelines" in result.stdout
        assert "- coding [Built-in]: Standards and conventions" in result.stdout
        assert "- testing [Built-in]: Best practices for writing" in result.stdout
        assert "- vcs [Built-in]: Guidelines for using version control" in result.stdout
