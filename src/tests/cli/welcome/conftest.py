...
RESOURCES_DIR = (
    Path(__file__).parent.parent.parent.parent / "pm" / "resources"
)  # Go up levels from tests/cli/welcome
...
try:
    ...
    if DEFAULT_GUIDELINE_PATH.is_file():
        pm_lines = DEFAULT_GUIDELINE_PATH.read_text(encoding="utf-8").splitlines()
    ...
    if CODING_GUIDELINE_PATH.is_file():
        ...
        last_line = (
            CODING_GUIDELINE_PATH.read_text(encoding="utf-8").splitlines()[-1].strip()
        )
    ...
    if TESTING_GUIDELINE_PATH.is_file():
        ...
        testing_lines = TESTING_GUIDELINE_PATH.read_text(encoding="utf-8").splitlines()
