...
def test_init_interactive_default_guideline_selection(runner: CliRunner):
    ...
    with runner.isolated_filesystem() as fs:
        ...
        assert (
            "Applying default guideline 'pm'." in result.stdout
        )  # Check default message
...
def test_init_interactive_specific_guideline_selection(runner: CliRunner):
    ...
    with runner.isolated_filesystem() as fs:
        ...
        result = runner.invoke(cli, ["init"], input=input_str, catch_exceptions=False)
        ...
        assert config_data["guidelines"]["active"] == [
            "coding",
            "vcs",
        ], f"Specific guidelines not set correctly in:\n{config_content}"
...
def test_init_interactive_rerun_keep_selection(runner: CliRunner):
    ...
    with runner.isolated_filesystem() as fs:
        ...
        result1 = runner.invoke(cli, ["init"], input=input1, catch_exceptions=False)
        assert result1.exit_code == 0, "First init run failed"
        config_path = fs_path / PM_DIR_NAME / "config.toml"
        assert config_path.is_file(), "Config file not created on first run"
        try:
            config_data1 = toml.loads(config_path.read_text())
        ...
        result2 = runner.invoke(cli, ["init"], input=input2, catch_exceptions=False)
        ...
        assert (
            "Keeping current guideline selection." in result2.stdout
        )  # Check keep message

        # Verify config file content is unchanged
        try:
            config_content2 = config_path.read_text()
            config_data2 = toml.loads(config_content2)
        except Exception as e:
            pytest.fail(
