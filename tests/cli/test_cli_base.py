...
def test_is_valid_uuid():
    ...
    assert common_utils.is_valid_uuid(str(uuid.uuid4())) is True
    assert common_utils.is_valid_uuid("not-a-uuid") is False
    assert common_utils.is_valid_uuid("project-slug") is False
    assert common_utils.is_valid_uuid("") is False


# --- Test resolve_project_identifier ---


@patch.object(common_utils, "get_project")
@patch.object(common_utils, "get_project_by_slug")
def test_resolve_project_by_uuid(mock_get_by_slug, mock_get_by_id):
    ...
    resolved_project = common_utils.resolve_project_identifier(mock_conn, project_id)
...
def test_resolve_project_by_slug(mock_get_by_slug, mock_get_by_id):
    ...
    resolved_project = common_utils.resolve_project_identifier(mock_conn, project_slug)
...
def test_resolve_project_not_found(mock_get_by_slug, mock_get_by_id):
    ...
    with pytest.raises(
        click.UsageError, match=f"Project not found with identifier: '{identifier}'"
    ):
        common_utils.resolve_project_identifier(mock_conn, identifier)
...
@patch.object(common_utils, "get_task")
@patch.object(common_utils, "get_task_by_slug")
def test_resolve_task_by_uuid(mock_get_by_slug, mock_get_by_id):
    ...
    mock_task = Task(
        id=task_id,
        project_id=mock_project_obj.id,
        name="Test Task",
        slug="test-task",
    ...
    resolved_task = common_utils.resolve_task_identifier(
...
def test_resolve_task_by_slug(mock_get_by_slug, mock_get_by_id):
    ...
    mock_get_by_id.assert_not_called()  # Because it's not a UUID
    mock_get_by_slug.assert_called_once_with(mock_conn, mock_project_obj.id, task_slug)
    assert resolved_task == mock_task
...
def test_resolve_task_uuid_wrong_project(mock_get_by_slug, mock_get_by_id):
    ...
    with pytest.raises(
        click.UsageError,
        match=f"Task not found with identifier '{task_id}' in project '{mock_project_obj.name}'",
    ):
        common_utils.resolve_task_identifier(mock_conn, mock_project_obj, task_id)

    mock_get_by_id.assert_called_once_with(mock_conn, task_id)
    # It should try slug lookup after ID check fails due to wrong project
    mock_get_by_slug.assert_called_once_with(mock_conn, mock_project_obj.id, task_id)
...
def test_resolve_task_slug_wrong_project(mock_get_by_slug, mock_get_by_id):
    ...
    with pytest.raises(
        click.UsageError,
        match=f"Task not found with identifier '{task_slug}' in project '{mock_project_obj.name}'",
    ):
        common_utils.resolve_task_identifier(mock_conn, mock_project_obj, task_slug)
...
def test_resolve_task_not_found(mock_get_by_slug, mock_get_by_id):
    ...
    with pytest.raises(
        click.UsageError,
        match=f"Task not found with identifier '{identifier}' in project '{mock_project_obj.name}'",
    ):
        common_utils.resolve_task_identifier(mock_conn, mock_project_obj, identifier)

    mock_get_by_id.assert_not_called()
    mock_get_by_slug.assert_called_once_with(mock_conn, mock_project_obj.id, identifier)
